...
The `--cleanup` flag will not process new posts for you. If you wish to process new posts and clean up stale posts at the same time, please use `-u`/`--update`.
...
graph TB
    ...
    D --> G[Jekyll _posts]
    E --> G
    F --> H[Jekyll assets/images/obsidian]
```