...
```
This will, after asking for your confirmation, remove all the files with filename of the form `yyyy-mm-dd-{filename}.md` (i.e. posts generated by this tool) in the destination folder whose corresponding file is not in the publication folder. It will also remove outdated posts (i.e., if you manually add a new `date` in the frontmatter or somehow change the creation date of your post).
